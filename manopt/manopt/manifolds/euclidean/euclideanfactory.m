...
%       Added functionality to handle multidimensional arrays.
    ...
    % MS, 13.06.2020: use MATLAB's dot function instead:
    M.inner = @(x, d1, d2) dot( d1(:), d2(:) );
    ...
    % Marco, 30.04.2019: I added this trivial function in order to avoid
    % to create two different files for the Modified Goldstein-like
    % condition.
    M.inv_retr = @log;
    function y = log(x, d)
        y = d - x;
