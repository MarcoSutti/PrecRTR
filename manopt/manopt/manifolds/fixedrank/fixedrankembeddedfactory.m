function M = fixedrankembeddedfactory(m, n, k, retr_type)
...
%       desired effect.
    ...
    % MS, 13.06.2020:
    M.inner = @(x, d1, d2) dot( d1.M(:), d2.M(:) ) + dot( d1.Up(:), d2.Up(:) ) + dot( d1.Vp(:), d2.Vp(:) );
    ...
    % Marco, 09.04.2019.
    % Choose retraction and its inverse, as specified by 'retr_type'.
    if strcmp(retr_type, 'metric') == 1
        M.retr = @metric_retraction;
        M.inv_retr = @inv_metric_retr_fixed_point;
    elseif strcmp(retr_type, 'metric_explicit_inverse') == 1
        M.retr = @metric_retraction;
        M.inv_retr = @inv_metric_retr_explicit_formula;    
    elseif strcmp(retr_type, 'ortho_zhang') == 1
        M.retr = @retraction_orthographic;
        M.inv_retr = @inv_retraction_orthographic;
    elseif strcmp(retr_type, 'ortho_ao') == 1
        M.retr = @retr_orth_AO2015;
        M.inv_retr = @inv_retr_orth_AO2015;
    else
        error('Please select a valid retraction type:\n%s', ' ''metric'', ''metric_explicit_inverse'', ''ortho_ao'', ''ortho_zhang''.');
    end
    M.retr_type = retr_type;
    
    % Marco, 09.04.2019.
    % Orthographic retraction provided by Absil and Oseledets.
    % See Absil and Oseledets, 2015.
    % Downloaded from https://sites.uclouvain.be/absil/2013.04.
    function Y = retr_orth_AO2015(X, Z, t)
        if nargin < 3
            t = 1.0;
        end
        
        if M.norm(X, Z) < eps     
            % Marco, 23.07.2019: this fixes a problem occurring when the
            % tangent vector Z is numerically zero.
            Y.U = X.U;
            Y.V = X.V;
            Y.S = X.S;
        else
            [ YUt, Ru ] = qr( X.U*( X.S + t*Z.M ) + t*Z.Up, 0 );
            [ YVt, Rv ] = qr( X.V*( X.S + t*Z.M )' + t*Z.Vp, 0 );
            
            %             if cond(X.S+t*Z.M) > 1e15
            %                 warning('X.S+t*Z.M is badly conditioned, cond(X.S+t*Z.M) = %.3e.\n', cond(X.S+t*Z.M) );
            %                 %                 cond(X.S+t*Z.M + 100*eps*eye(size(Z.M)))
            %                 %                 X.S+t*Z.M
            %             end
            
            [ Ut, St, Vt ] = svd( Ru * (( X.S+t*Z.M )\Rv') );
            Y.U = YUt * Ut;
            Y.V = YVt * Vt;
            Y.S = St + eps * eye( size( St, 1 ) );   % Was this really necessary?
        end

    end


    % Marco, Changed: 03.07.2019.
    % Inverse of the orthographic retraction of Absil and Oseledets.
    % See Absil and Oseledets, 2015.
    % The inverse of the orthographic retraction is equal to the projection
    % of (Y-X) onto the tangent space at X. Here, we exploit the "tangent
    % vector format" to compute it efficiently.
    % See §3.2.3 Inverse retraction of Absil and Oseledets, 2015.
    function Z = inv_retr_orth_AO2015( X, Y )
      
        Z = projection(X, Y);
        Z.M = Z.M - X.S;

        % For debugging purposes:        
        %         Retr_X_Z = retr_orth_AO2015(X, Z);
        %
        %         res = ( Retr_X_Z.U * Retr_X_Z.S * Retr_X_Z.V' - Y.U * Y.S * Y.V' );
        %         norm_res = norm( res, 'fro' )/norm(Y.S,'fro');
        %         if norm_res > 1e-6
        %             fprintf('Last norm_res = %.8e\n', norm_res );
        %             cprintf('_red', 'inverse orthographic retraction is not correct!\n');
        %             pause
        %         end
    end
    ...
    %
    % M.retr = @retraction;
    function Y = metric_retraction(X, Z, t)
        if nargin < 3
            t = 1.0;
        end
    ...
    % Marco, 12.04.2019: Function to convert from tangent vector format to
    % a matrix.
    M.tangent2matrix = @tangent2matrix;
    function Zamb_mat = tangent2matrix( X, Z )
