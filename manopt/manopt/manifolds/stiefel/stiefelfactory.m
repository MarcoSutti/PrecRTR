...
%     end
    ...
    % MS, 20.08.2020: Retraction based on the polar decomposition.
    M.retr_polar = @retraction_polar;
    function P_St_Z = retraction_polar(X, xi, t)
        %         if nargin < 3
        %             Y = X + U;
        %         else
        %             Y = X + t*U;
        ...
        if nargin < 3
            Z = X + xi;
            ZtZ = Ip + xiTxi;   % MS: this expression is equivalent to Z'*Z, because xi'*X + X'*xi = 0;
        else
            Z = X + t * xi;
            ZtZ = Ip + t^2 * xiTxi;
        end
