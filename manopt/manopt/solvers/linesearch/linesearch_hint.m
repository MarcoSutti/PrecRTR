...
%       Now using storedb.remove() to keep the cache lean.
    ...
    if canGetLinesearch(problem)
        [ alpha, stat_hzls ] = getLinesearch(problem, x, d, storedb, key);
    ...
    cost_evaluations = 1 + stat_hzls.nf;    % Original version: cost_evaluations = 1;
    % Marco, 15.11.2019: sum the number of cost function evaluations done
    % inside Hager-Zhang line search.
    ...
    % Marco, 17.04.2019. Take half of the stepsize.
    alpha = (1/2) * alpha;   % 26.05.2019: stepsize must be tuned...
    % and look closer down the line.
    storedb.remove(newkey);              % we no longer need this cache
    newx = problem.M.retr(x, d, alpha);
    newkey = storedb.getNewKey();
