...
% %--------------------------------------------------------------------------
% % MS's addition, Sept 2022.
% %--------------------------------------------------------------------------
% global Norm_Res;             % MS, 2022.09.27.
% global Wh_ref_sol_global;    % MS, 2022.09.28.
% global norm_Wh_ref_global;   % MS, 2022.09.28.
% global idmg_red_global;      % MS, 2022.09.29.
% global err_W_array;          % MS, 2022.09.28.
% global W5;                   % MS, 2022.10.19.
% %--------------------------------------------------------------------------
...
% % MS's addition, Sept 2022.
% %--------------------------------------------------------------------------
% Norm_Res = [ Norm_Res, norm_struct( problem.egrad( x ) ) ];   % MS, 2022.09.27.
% 
% % MS, 2022.09.28.
% % Error with respect to reference solution, called err-W in the paper:
% err_W = norm( x.U * x.S * x.V' + W5.U * W5.S * W5.V'- Wh_ref_sol_global, 'fro' )/norm_Wh_ref_global;
% err_W_array = [ err_W_array, err_W ];
% idmg_red_global = [ idmg_red_global, 1 ];
% %--------------------------------------------------------------------------
...
%     %------------------------------------------------------------------
%     Norm_Res = [ Norm_Res, norm_struct( problem.egrad( x_prop ) ) ];   % MS, 2022.09.27.
%     % MS, 2022.09.28.
%     % Error with respect to reference solution, called err-W in the paper:
%     err_W = norm( x_prop.U * x_prop.S * x_prop.V' + W5.U * W5.S * W5.V' - Wh_ref_sol_global, 'fro' )/norm_Wh_ref_global;
%     err_W_array = [ err_W_array, err_W ];
%     idmg_red_global = [ idmg_red_global, 0 ];
%     %------------------------------------------------------------------
    ...
    %----------------------------------------------------------------------
    % MS, added on 2023.02.23.
    %----------------------------------------------------------------------
    % We kill the trust-region method if the cumulative number of inner
    % iterations of the tCG solver becomes excessive.
    cumul_sum_numinner = 0;
    max_numinner = 0;
    for i=1:k+1
        cumul_sum_numinner = cumul_sum_numinner + info(i).numinner;
        if info(i).numinner > max_numinner
            max_numinner = info(i).numinner;
        end
    end

    if cumul_sum_numinner > 30000
        fprintf('The cumulative sum of inner iterations of tCG has exceeded the max allowed. \n')
        fprintf( 'cumul_sum_numinner = %.d.\n', cumul_sum_numinner )
        fprintf( 'max_numinner = %.d.\n', max_numinner )
        fprintf('Exiting RTR...')
        stats
        return
    end
